...
{
    public partial class FlowScript : PageBase
    {
        ...
        {
            IQueryable<AppBox.Script> q = DB.Scripts; //.Include(u => u.Dept);
            ...
            // 排列和数据库分页
            q = SortAndPage<AppBox.Script>(q, Grid1);
        ...
        private void BindGrid2()
        {
            ...
            AppBox.Flow q = DB.Flows.Include(r => r.Scripts)
               .Where(r => r.ID == id)
               .FirstOrDefault();
            IQueryable<AppBox.Script> x = q.Scripts.AsQueryable();
            // 在查询添加之后，排序和分页之前获取总记录数
            Grid2.RecordCount = x.Count();

            // 排列和数据库分页
            x = SortAndPage<AppBox.Script>(x, Grid2);
        ...
        protected void Left_Click(object sender, EventArgs e)
        {
            ...
            AppBox.Flow flow = DB.Flows.Include(r => r.Scripts)
                 .Where(r => r.ID == id)
                 .FirstOrDefault();

            //flow.Scripts.Clear();

            foreach (int scriptID in ids)
            {
                AppBox.Script script = Attach<AppBox.Script>(scriptID);
                flow.Scripts.Add(script);
            }
            DB.SaveChanges();
